...
class UserInputGroupView extends StatefulWidget {
  @override
  _UserInputGroupViewState createState() => _UserInputGroupViewState();
}

class _UserInputGroupViewState extends State<UserInputGroupView> {
  ...
  Future getUsesrFcm() async {
    FirebaseFirestore.instance
      ...
      querySnapshot.docs.forEach((doc) {
        if (doc.id != con.currentUser?.uid.toString()) {
          aGroupController.usersCollectionFcm.add(doc['fcmToken']);
        }
      });
    });
  }
  ...
  @override
  void dispose() {
    aGroupController.usersCollectionFcm.clear();
                    ...
                    child: Row(
                      children: [
                        ...
                        InkWell(
                          onTap: () {
                            Map mp;
                            int admincounter = aGroupController.adminCount;
                            if (controller.text.isNotEmpty) {
                              // add user message  conter
                              admincounter++;

                              mp = aGroupController.groupMembersStatusMap;
                              mp.forEach((key, value) {
                                if (key != con.currentUser?.uid.toString()) {
                                  mp[key] = value + 1;
                                } else {
                                  mp[key] = 0;
                                }
                              ...
                              aGroupController.adminCount = admincounter;

                              log('map meber count ${aGroupController.groupMembersStatusMap}');
                              ...
                              fireStore
                                  ...
                                  .doc(aGroupController.groupName)
                                  .update({
                                'AdminCount': aGroupController.adminCount,
                                'GroupMembers':
                              ...
                              LocalNotificationServices.sendGroupNotification(
                                aGroupController.usersCollectionFcm,
                                'nuovo messaggio',
                                controller.text,
                              );
